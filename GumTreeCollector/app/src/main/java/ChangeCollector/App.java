...
public class App {
    ...
    public static void main(String[] args) {
        ...
        File[] closure = app.extractOnlyClosure(subdir);

        for (File dir : closure) {
            System.out.println(dir);

            List<File> fileList = traverseDirectory(dir);

            for (File file : fileList) {
                System.out.println("File: " + file.getAbsolutePath());
            }
            
            String result = app.extract_diff(fileList.get(0).toString(), fileList.get(1).toString());
            
            try {
                String saveFiles = dir.toString();
                saveFiles = saveFiles.replaceFirst("prepare_pool", "gumTrees");
                // System.out.println(saveFiles);

                BufferedWriter writer = new BufferedWriter(new FileWriter(saveFiles + "/gumtree.txt", false));
                writer.write(result + "\n");
                writer.close();
            }
            catch (Exception e) {
    ...
    public String extract_diff(String new_file, String old_file) {
        String result;
        System.out.println("[status] > extracting diff from files" + new_file + old_file);

        Extractor extractor = new Extractor();
        result = extractor.extract_gumtree_log(new_file, old_file);
       
        return result;
    }

    public File[] extractOnlyClosure(File[] subdir){

        int count = 0;
        for (File dir : subdir) {
            if (dir.toString().contains("Closure")) {
                count ++;
            }
        }

        File[] closure = new File[count];
        int i = 0; //to store it into closure 
        for (File dir : subdir) {
            if (dir.toString().contains("Closure")) {
                closure[i] = dir;
                i++;
    ...
    public static List<File> traverseDirectory(File directory) {
        ...
        if (files != null) {
            for (File file : files) {
                if (file.isDirectory()) {
                    fileList = traverseDirectory(file); // Recursive call to traverse subdirectories
                } else if (file.isFile()) {
                    fileList.add(file);
                }
            }
